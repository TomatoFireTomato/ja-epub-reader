...
// 返回 { words:[{surface,reading,pos,lemma}], grammar:[{point,detail}] }
// - words：kuromoji 分出的实词（去掉标点、助词、助动词），去重。
// - grammar：用句型匹配器识别的常见 JLPT 句型（～ている 等），自带中文解释与例句。
export async function segmentLocal(sentence) {
  ...
  for (const t of tokens) {
    ...
    seenW.add(key)
    words.push({ surface, reading, pos: POS_MAP[t.pos] || t.pos || '', lemma: t.basic_form && t.basic_form !== '*' ? t.basic_form : surface })
  }
  return { words, grammar: detectGrammar(sentence) }
}
