...
// ---------- 四个小任务（按需调用，省 token） ----------
// 句子语言自动判定：日文用「日语教师」提示，英文用「英语老师」提示。
...
const SEG_SYSTEM_EN = `你是英语老师。请对给定英文句子做分词与语法点识别，只识别、不要解释。
...
export function segmentSentence(sentence) {
  ...
  // 本地分词：离线、零 token（日文 kuromoji + 句型；英文按词边界切）
  if (settings.value.localSegment) return lang === 'en' ? segmentLocalEn(sentence) : segmentLocal(sentence)
  return completeJson(lang === 'en' ? SEG_SYSTEM_EN : SEG_SYSTEM, `句子：「${sentence}」`)
}
...
const WORD_SYSTEM_EN = `你是英语老师，向中文母语学习者讲解。给定英文句子和其中一个词，只解释这一个词。
只输出 JSON：{"reading":"音标(可选)","lemma":"词典原形","pos":"中文词性","meaning":"该语境下的中文释义","note":"用法/搭配/时态等说明(可选)"}
用简体中文，只输出 JSON。`

export function explainWord(sentence, surface) {
  const en = detectLang(sentence) === 'en'
  return completeJson(en ? WORD_SYSTEM_EN : WORD_SYSTEM, `句子：「${sentence}」\n要解释的词：「${surface}」`)
}
...
const GRAMMAR_SYSTEM_EN = `你是英语老师，向中文母语学习者讲解。给定英文句子和其中一个语法点，解释它。
只输出 JSON：{"explanation":"中文解释其含义与用法","example":"简短英文例句(可选)"}
用简体中文，只输出 JSON。`

export function explainGrammar(sentence, point) {
  const en = detectLang(sentence) === 'en'
  return completeJson(en ? GRAMMAR_SYSTEM_EN : GRAMMAR_SYSTEM, `句子：「${sentence}」\n语法点：「${point}」`)
}
...
const TRANSLATE_SYSTEM_EN = `你是英语翻译。把给定英文句子翻译成自然的简体中文。
只输出 JSON：{"translation":"中文翻译"}
只输出 JSON。`

export function translateSentence(sentence) {
  const en = detectLang(sentence) === 'en'
  return completeJson(en ? TRANSLATE_SYSTEM_EN : TRANSLATE_SYSTEM, `句子：「${sentence}」`)
}
