...
import { llmGenerate } from './localllm.js'
...
export function explainWord(sentence, surface) {
  if (settings.value.localLLM) {
    return llmGenerate(
      '你是日语老师，用简体中文简短讲解日语词：给出假名读音、词义、必要时活用/用法。直接给解释，不寒暄、不超过 3 行。',
      `句子：「${sentence}」\n请解释其中的「${surface}」。`
    ).then((t) => ({ meaning: t }))
  }
...
export function explainGrammar(sentence, point) {
  if (settings.value.localLLM) {
    return llmGenerate(
      '你是日语老师，用简体中文简短解释日语语法点的含义与用法，可给一个简短例句。直接给解释，不寒暄。',
      `句子：「${sentence}」\n请解释语法点「${point}」。`
    ).then((t) => ({ explanation: t }))
  }
...
export function translateSentence(sentence) {
  if (settings.value.localLLM) {
    return llmGenerate(
      '你是日语翻译。把日语句子翻译成自然的简体中文，只输出译文，不要解释、不要原文。',
      `「${sentence}」`
    ).then((t) => ({ translation: t }))
  }
